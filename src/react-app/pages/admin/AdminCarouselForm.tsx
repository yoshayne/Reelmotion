...
import type { CarouselItem, Video, Series } from "@/shared/types";
...
export default function AdminCarouselForm() {
  ...
  const [linkType, setLinkType] = useState<"none" | "video" | "series">("none");
  ...
  useEffect(() => {
    if (!isCreator) return;
    // Load available videos and series for the picker
    Promise.all([
      apiFetch("/api/admin/videos").then(r => r.json()).catch(() => []),
      apiFetch("/api/admin/series").then(r => r.json()).catch(() => []),
    ]).then(([v, s]) => {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/admin/carousel")
      ...
      .then((list) => {
        ...
        if (item) {
          setForm(item);
          if (item.video_id) setLinkType("video");
          else if (item.series_id) setLinkType("series");
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm((prev) => ({ ...prev, image_url: data.url || `/api/images/${data.key}` }));
    setUploading(false);
  };

  const handleLinkTypeChange = (type: "none" | "video" | "series") => {
    setLinkType(type);
    // Clear both IDs, then let the user pick
    setForm((p) => ({ ...p, video_id: null, series_id: null }));
  };
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-lg mx-auto px-4 pb-16" style={{ paddingTop: "calc(max(env(safe-area-inset-top), 0px) + 5rem)" }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Link to content */}
          <div>
            <label className="block text-sm text-gray-400 mb-2">Link "Watch Now" to</label>
            <div className="flex gap-2 mb-3">
              {(["none", "video", "series"] as const).map((t) => (
                <button
                  key={t}
                  type="button"
                  ...
                  style={{
                    backgroundColor: linkType === t ? '#E8001D' : 'rgba(255,255,255,0.07)',
                    color: linkType === t ? '#fff' : 'rgba(255,255,255,0.5)',
                  }}
                >
                  {t === "none" ? "No link" : t}
                </button>
            ...
            {linkType === "video" && (
              <select
                value={form.video_id ?? ""}
                onChange={(e) => setForm((p) => ({ ...p, video_id: e.target.value ? Number(e.target.value) : null, series_id: null }))}
                className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-red-600"
              >
                <option value="">— Select a video —</option>
                {videos.map((v) => (
                  <option key={v.id} value={v.id}>
                    {v.title}{v.content_type === "episode" && v.episode_number ? ` · E${v.episode_number}` : ""}
                  </option>
                ))}
              </select>
            )}
            ...
            {linkType === "series" && (
              <select
                ...
                onChange={(e) => setForm((p) => ({ ...p, series_id: e.target.value ? Number(e.target.value) : null, video_id: null }))}
                className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-red-600"
              >
            ...
            {linkType !== "none" && (
              <p className="text-xs text-gray-500 mt-1.5">
                {linkType === "video"
                  ? "Watch Now goes to the video player for the selected film."
                  : "Watch Now goes to the series page for the selected series."}
              </p>
            )}
          </div>
